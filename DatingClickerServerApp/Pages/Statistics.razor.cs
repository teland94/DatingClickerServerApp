        ...
        private Task<StatisticsResult> GetStatistics(DateTime startDate, bool isVerified, bool includeInactive)
        {
            ...
            var query = $@"
                SELECT 
                    ...
                    COUNT(CASE WHEN 'it' = ANY(string_to_array(LOWER(""Interests""), ',')) THEN 1 END) AS ""{nameof(StatisticsResult.UsersWithITInterests)}"",
                    COUNT(CASE WHEN 'it' = ANY(string_to_array(LOWER(""Interests""), ',')) AND ""HasChildren"" = false THEN 1 END) AS ""{nameof(StatisticsResult.UsersWithITInterestsNoChildren)}""
